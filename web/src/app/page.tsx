...
import { Play, Loader2, Github } from "lucide-react";
...
export default function PlaygroundPage() {
  ...
  const handleRun = useCallback(async () => {
    ...
    try {
      ...
      const trace = buildTrace(result, source);
      setPipeline((prev) => ({
...
function Header({
  ...
  return (
    <header className="flex items-center justify-between border-b border-border/60 bg-background/40 px-4 py-2.5 backdrop-blur">
      ...
      <div className="flex items-center gap-2">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex h-8 items-center gap-1.5 rounded-md border border-input px-2.5 text-xs text-muted-foreground hover:bg-accent hover:text-accent-foreground"
        >
...
function buildTrace(
  result: { output: string; error: RotError | null; timings: { lexMs: number; parseMs: number; interpretMs: number } },
  _source: string,
): string {
  const lines: string[] = [];
